...
function NewsPanel({ title, query, icon, onBack, country }) {
  ...
  useEffect(() => {
    ...
    if (!apiKey) { setError("API key not configured. Add VITE_ANTHROPIC_API_KEY to Vercel environment variables."); setLoading(false); return; }
    fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: "user", content: `Search the web RIGHT NOW for the very latest ${query}. Find real current articles from today or this week. Return ONLY a raw JSON array (no markdown, no backticks) of exactly 5 articles each with: title, summary (2 sentences), source, sentiment (positive/negative/neutral).` }]
      })
    })
      .then(r => r.json())
      .then(data => {
